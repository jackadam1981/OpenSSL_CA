...
function Create_RootCA_PEM() {
	...
	echo "转换CA P12，Windows双击导入证书，提示输入密码，可以为空。"
...
function Create_Server_PEM() {
    ...
    echo "中间CA 签名server san证书"
	read -p "交互生成，请注意选y" 
	openssl ca -config $PowerCA_Conf \
	...
	openssl x509 \
		...
		-CA $RootCA_cert \
		-CAkey $RootCA_KEY \
		-out $Server_PATH/${SERVERCN}_CA_x509.crt \
		-CAcreateserial \
		-days $((365 * year)) \
		-sha256 \
	...
	echo "PowerCA x509签发server san证书"
	openssl x509 \
		-req \
		-in $Server_PATH/$SERVERCN.csr \
		-CA $PowerCA_cert \
		-CAkey $PowerCA_KEY \
		-out $Server_PATH/${SERVERCN}_PowerCA_x509.crt \
		-CAcreateserial \
		-days $((365 * year)) \
		-sha256 \
		-extfile $Conf_Path/Server.ext \
...
function main() {
    ...
    read -p "是否生成服务器证书并用中间CA签名,直接回车，默认不生成。y/n?" isServer
    : ${isServer:="n"}
    if [ $isServer = "y" ]; then
        Create_Server_PEM
    fi
